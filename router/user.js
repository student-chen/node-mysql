...
module.exports = app => {
  ...
  router.post('/searchUser', async (req, res) => {
    ...
    // const { status, address, sex } = req.body
    searchData = await User.findAll({ raw: true })
    if(!searchData.length) return res.send({ status: 200, message: '暂无数据', resultValue: false, type: 'success'})
    res.send({ status: 200, message: '已更新数据', resultValue: searchData, type: 'success'})
    res.end()
  })
   ...
   * 调价路由校验是否已登录中间件：isNeedLoginMiddleWare()
   */
